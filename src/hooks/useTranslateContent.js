...
export const useTranslateContent = (
	...
	const getTranslateYValue = useCallback(() => {
		...
		if (
			scrollY + innerHeight >= startingY * 0.3 &&
			scrollY < startingY * 1.7
		) {
			// 'componentOffsetMultiplier' handles some weird cases when scrollY is less than the innerHeight
			const componentOffsetMultiplier = _.clamp(
				1 - scrollY / innerHeight,
				0,
				1
			);
			...
			const relativeY =
				...
				innerHeight -
				startingY -
				componentOffsetMultiplier * (componentHeight / 2);
